...
Актуальное состояние проекта, план и история итераций — в **`ROADMAP.md`** (корень репозитория).
Начинайте любой новый чат с чтения этого файла. После каждой итерации обновлять `ROADMAP.md`
(разделы «Текущий этап» и «Сделано»), а не этот документ.
